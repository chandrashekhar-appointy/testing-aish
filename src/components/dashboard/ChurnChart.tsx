...
// Mock data generator for churned customers
const generateMockData = (filters: Filters): ChurnedCustomer[] => {
  const customers: ChurnedCustomer[] = [];
  ...
  const churnReasons = ['Price Too High', 'Poor Support', 'Feature Gaps', 'Competitor Switch', 'Business Closure', 'Technical Issues'];
  
  for (let i = 1; i <= 80; i++) {
    const membership = memberships[Math.floor(Math.random() * memberships.length)];
    const subscriptionValue = 
      membership === 'Enterprise' ? Math.floor(Math.random() * 500) + 200 :
      membership === 'Premium' ? Math.floor(Math.random() * 200) + 100 :
      membership === 'Pro' ? Math.floor(Math.random() * 100) + 50 :
      Math.floor(Math.random() * 50) + 20;
    
    const customer: ChurnedCustomer = {
      appointyId: `CHR${i.toString().padStart(4, '0')}`,
      userName: `Churned User ${i}`,
      businessName: `Business ${i}`,
      profession: professions[Math.floor(Math.random() * professions.length)],
      country: countries[Math.floor(Math.random() * countries.length)],
      membership,
      staffCount: Math.floor(Math.random() * 20) + 1,
      monthlyAppointmentCount: Math.floor(Math.random() * 80) + 5,
      lastActiveDate: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      churnDate: new Date(Date.now() - Math.random() * 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      churnReason: churnReasons[Math.floor(Math.random() * churnReasons.length)],
      subscriptionValue
    };
...
const calculateBayesianNormal = (appointments: number, accounts: number): number => {
  const prior = 15; // prior belief about average appointments for churned customers (lower than active)
  const priorCount = 5; // confidence in prior
  return (appointments + prior) / (accounts + priorCount);
};

const ChurnChart: React.FC<ChurnChartProps> = ({ filters }) => {
  ...
  const chartData: ChartDataPoint[] = React.useMemo(() => {
    ...
    return Array.from(grouped, ([category, customerList]) => {
      ...
      const lostRevenue = customerList.reduce((sum, c) => sum + c.subscriptionValue, 0);
  ...
  useEffect(() => {
    ...
    // Left Y-axis for churned accounts (bars)
    ...
    // Create bars for churned accounts
    const accountBars = g.selectAll(".account-bar")
      ...
      .on("mouseover", function(event, d) {
        ...
        tooltip.html(`
          ...
          <div style="color: #fca5a5;">Churned Accounts: ${d.accounts.toLocaleString()}</div>
          <div style="color: #34d399;">Total Appointments: ${d.appointments.toLocaleString()}</div>
          <div style="color: #d4af37;">Bayesian Score: ${d.bayesianNormal.toFixed(2)}</div>
        `)
    ...
    // Add churned account value labels
    ...
    // Add left Y axis (churned accounts)
    g.append("g")
    ...
    g.append("text")
      ...
      .style("font-weight", "bold")
      .style("fill", "#991b1b")
      .text("Churned Accounts & Appointments");
    ...
    legend.append("text")
      ...
      .style("fill", "#991b1b")
      .text("Churned Accounts (Red Bars)");
    ...
    legend.append("text")
      ...
      .style("font-weight", "600")
      .style("fill", "#78350f")
      .text("Bayesian Normal Score (Brown Line)");
    ...
    legend.append("text")
      ...
      .style("fill", "#6b7280")
      .text("Left Axis: Churned Accounts & Appointments | Right Axis: Bayesian Normal Score");
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Analyze customer churn patterns. The chart shows churned accounts (red bars) and bayesian normal values (brown line).
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Customer Churn by {getViewModeLabel(viewMode)}
          </CardTitle>
          <CardDescription>
            Dual-axis visualization: Red bars show churned accounts (left axis), Brown line shows bayesian normal values (right axis)
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Lost Appointments</CardDescription>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Churned Accounts</CardDescription>
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-2xl text-red-800">
              ${chartData.reduce((sum, d) => sum + d.lostRevenue, 0).toLocaleString()}
            </CardTitle>
          </CardHeader>
        </Card>
        <Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Churned Customer Details</CardTitle>
          <CardDescription>
            Complete churn analysis with revenue impact and reason tracking
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-left p-2">Lost Appointments</th>
                      <th className="text-left p-2">Lost Revenue</th>
                      <th className="text-left p-2">Churn Reason</th>
                      <th className="text-left p-2">Churn Date</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={customer.appointyId} className="border-b hover:bg-muted/50">
                        ...
                        <td className="p-2 text-sm">
                          <span className={`px-2 py-1 rounded text-xs ${
                            customer.membership === 'Enterprise' ? 'bg-red-200 text-red-900' :
                            customer.membership === 'Premium' ? 'bg-red-150 text-red-800' :
                            customer.membership === 'Pro' ? 'bg-red-100 text-red-700' :
                        ...
                        <td className="p-2 text-sm">
                          <span className={`px-2 py-1 rounded text-xs ${
                            customer.churnReason === 'Price Too High' ? 'bg-orange-100 text-orange-800' :
                            customer.churnReason === 'Poor Support' ? 'bg-red-100 text-red-800' :
                            customer.churnReason === 'Feature Gaps' ? 'bg-blue-100 text-blue-800' :
                            customer.churnReason === 'Competitor Switch' ? 'bg-purple-100 text-purple-800' :
                            customer.churnReason === 'Business Closure' ? 'bg-gray-100 text-gray-800' :
                            'bg-yellow-100 text-yellow-800'
                          }`}>
                            {customer.churnReason}
                          </span>
                        </td>
                        <td className="p-2 text-sm">{customer.churnDate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-muted-foreground">
                  Showing {startIndex + 1} to {Math.min(endIndex, customers.length)} of {customers.length} churned customers
                </div>
                <div className="flex gap-2">
                  <Button
                    variant="outline"
