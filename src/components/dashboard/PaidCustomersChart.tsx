...
interface PaidCustomersChartProps {
  filters: Filters;
}
...
// Mock data generator for paid customers
const generateMockData = (filters: Filters): PaidCustomer[] => {
  const customers: PaidCustomer[] = [];
  ...
  const statuses = ['Active', 'Pending', 'Canceled', 'Trial'];
  ...
  for (let i = 1; i <= 120; i++) {
    ...
    const customer: PaidCustomer = {
      appointyId: `PAY${i.toString().padStart(4, '0')}`,
      userName: `Paid User ${i}`,
      businessName: `Business ${i}`,
      profession: professions[Math.floor(Math.random() * professions.length)],
      country: countries[Math.floor(Math.random() * countries.length)],
      membership,
      staffCount: Math.floor(Math.random() * 20) + 1,
      monthlyAppointmentCount: Math.floor(Math.random() * 150) + 10,
      subscriptionValue,
...
const calculateBayesianNormal = (appointments: number, accounts: number): number => {
  const prior = 25; // prior belief about average appointments for paid customers
  const priorCount = 5; // confidence in prior
  return (appointments + prior) / (accounts + priorCount);
};

const PaidCustomersChart: React.FC<PaidCustomersChartProps> = ({ filters }) => {
  const svgRef = useRef<SVGSVGElement>(null);
  const [customers, setCustomers] = useState<PaidCustomer[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [loading, setLoading] = useState(false);
  const [viewMode, setViewMode] = useState<ViewMode>('membership');
  ...
  const chartData: ChartDataPoint[] = React.useMemo(() => {
    ...
    return Array.from(grouped, ([category, customerList]) => {
      ...
      const revenue = customerList.reduce((sum, c) => sum + c.subscriptionValue, 0);
  ...
  useEffect(() => {
    ...
    const accountBars = g.selectAll(".account-bar")
      ...
      .on("mouseover", function(event, d) {
        ...
        tooltip.html(`
          ...
          <div style="color: #60a5fa;">Paid Accounts: ${d.accounts.toLocaleString()}</div>
          <div style="color: #34d399;">Total Appointments: ${d.appointments.toLocaleString()}</div>
          <div style="color: #fbbf24;">Bayesian Score: ${d.bayesianNormal.toFixed(2)}</div>
        `)
    ...
    legend.append("text")
      ...
      .style("font-weight", "600")
      .style("fill", "#d97706")
      .text("Bayesian Normal Score (Orange Line)");
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select how to group your paid customers. The chart shows paid accounts (colored bars) and bayesian normal values (orange line).
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Paid Customers by {getViewModeLabel(viewMode)}
          </CardTitle>
          <CardDescription>
            Dual-axis visualization: Colored bars show paid accounts (left axis), Orange line shows bayesian normal values (right axis)
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Paid Accounts</CardDescription>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Paid Customer Details</CardTitle>
          <CardDescription>
            Complete subscription and payment information for paid customers
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <>
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-left p-2">Subscription Value</th>
                      ...
                      <tr key={customer.appointyId} className="border-b hover:bg-muted/50">
                        ...
                        <td className="p-2 text-sm">
                          <span className={`px-2 py-1 rounded text-xs ${
                            ...
                            customer.subscriptionStatus === 'Trial' ? 'bg-blue-100 text-blue-800' :
                            'bg-red-100 text-red-800'
                          }`}>
                            {customer.subscriptionStatus}
                          </span>
                        </td>
                        <td className="p-2 text-sm">{customer.paymentDate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-muted-foreground">
                  Showing {startIndex + 1} to {Math.min(endIndex, customers.length)} of {customers.length} paid customers
                </div>
                <div className="flex gap-2">
                  <Button
                    variant="outline"
...
export default PaidCustomersChart; 