...
interface ChartData {
  category: string;
  appointments: number;
  avgStaffPerAccount: number;
  bayesianNormal: number;
}

interface StaffCountDetailDashboardProps {
  isOpen: boolean;
  onClose: () => void;
...
const generateMockStaffData = (filters: DetailFilters): ChartData[] => {
  ...
  return categories.map(category => ({
    category,
    appointments: Math.floor(Math.random() * 1200) + 300,
    avgStaffPerAccount: Math.floor(Math.random() * 15) + 5, // 5-20 staff per account
    bayesianNormal: Math.random() * 0.9 + 0.1
  }));
};

const StaffCountDetailDashboard: React.FC<StaffCountDetailDashboardProps> = ({
  isOpen,
  onClose,
  filters,
  onFiltersChange
  ...
  useEffect(() => {
    if (isOpen) {
      setLoading(true);
      setTimeout(() => {
        const mockData = generateMockStaffData(filters);
  ...
  const drawChart = () => {
    ...
    const yScaleLeft = d3.scaleLinear()
      .domain([0, d3.max(chartData, d => Math.max(d.appointments, d.avgStaffPerAccount * 50)) || 0])
      .range([height, 0]);
    ...
    // Create bars for average staff per account (scaled up for visibility)
    g.selectAll('.bar-staff')
      .data(chartData)
      .enter().append('rect')
      .attr('class', 'bar-staff')
      ...
      .duration(800)
      .delay((d, i) => i * 100)
      .attr('y', d => yScaleLeft(d.avgStaffPerAccount * 50)) // Scale up for visibility
      .attr('height', d => height - yScaleLeft(d.avgStaffPerAccount * 50));
    ...
    // Add text labels for staff bars
    g.selectAll('.text-staff')
      .data(chartData)
      .enter().append('text')
      .attr('class', 'text-staff')
      .attr('x', d => (xScale(d.category) || 0) + xScale.bandwidth() / 2)
      .attr('y', d => yScaleLeft(d.avgStaffPerAccount * 50) - 5)
      .attr('text-anchor', 'middle')
      .style('font-size', '10px')
      .style('font-weight', 'bold')
      .style('fill', '#8b5cf6')
      .text(d => d.avgStaffPerAccount.toString())
      .attr('opacity', 0)
      .transition()
      .duration(500)
      .delay((d, i) => i * 100 + 800)
    ...
    const legendItems = [
      { name: 'Total Appointments', color: '#10b981' },
      { name: 'Avg Staff per Account', color: '#8b5cf6' },
    ...
    // Add hover effects
    g.selectAll('.bar-appointments, .bar-staff')
      .on('mouseover', function(event: any, d: unknown) {
        ...
        tooltip.html(`
          ...
          Total Appointments: ${data.appointments}<br/>
          Avg Staff per Account: ${data.avgStaffPerAccount}<br/>
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                Staff Count - Detailed Analysis
              </CardTitle>
              <CardDescription>
                Interactive dashboard showing total appointments, average staff per account, and bayesian normal metrics
              </CardDescription>
            </div>
          ...
          <CardContent>
            ...
            <div className="bg-white rounded-lg border p-4">
              <h4 className="font-semibold mb-4 text-gray-800">
                Staff Metrics by {filters.xAxis.charAt(0).toUpperCase() + filters.xAxis.slice(1)}
              </h4>
...
export default StaffCountDetailDashboard; 